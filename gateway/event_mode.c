...
volatile size_t num_sensores = 0;
...
{
    uint8_t data[PAYLOAD_TX_LENGTH] = {BROADCAST_ID, MODO_EVENTO, sensor_list[0].umbral, sensor_list[0].time, 0xFF};
...
int main(int argc, char *argv[])
{
    ...
    if(sensor.dev_id == 0xFF)
    {
        ...
        {
            sensor.dev_id = atoi(argv[i*5 + 2]);
            sensor.umbral = (int)(atof(argv[i*5 + 3]) * 100.0);
            sensor.lora_mode = atoi(argv[i*5 + 4]);
            sensor.rate = atoi(argv[i*5 + 5]);
            sensor.time = atoi(argv[i*5 + 6]);
    ...
    {
        sensor.umbral = (int)(atof(argv[2]) * 100.0);
